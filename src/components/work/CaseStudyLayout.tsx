...
export function CaseStudyLayout({
    ...
    // Full Page Hero Logic (Particle Life, Log Slice)
    const isFullPageHero = id === 'particle-life-131' || id === 'log-slice';
        ...
        <article className={cn(
            ...
            {isFullPageHero && hasVisualComponent(id || '') && (
                <div className="fixed inset-0 w-full h-screen z-0">
                    {id !== 'log-slice' && <div className="absolute inset-0 bg-background/20 z-10" />} {/* Minimal dimming */}
                    <div className="w-full h-full opacity-100">
                        <VisualManager projectId={id || ''} variant="complexity" />
                    </div>
                    {/* Gradient Fade at bottom to blend into content */}
                    {id !== 'log-slice' && <div className="absolute bottom-0 left-0 w-full h-64 bg-gradient-to-t from-background to-transparent z-20" />}
                </div>
            )}

            <div className={cn(
                id === 'log-slice' ? "relative z-10 w-full" : "container mx-auto px-6 md:px-12 pt-12 pb-32 max-w-screen-xl relative z-10"
            )}>
